...
def category_input():
    ...
    while(loop):
        try:
            ...
            else if(inputs[0] in categories):
                choosencategory.append(inputs[0])
            else:
                raise ValueError 

            #subcategory process. pretty similar to category one   
            if(category[0] in subcategories):

                print("This category has subcategories as listed below. please enter the subcategory or its number:")
                print("   ".join("{}. {}".format(i[0],i[1]) for i in enumerate(subcategories[choosencategory],1)))   
                inputs.append(input().title())

                if(inputs[1].isnumeric())
                    choosencategory.append(subcategories[choosencategory][int(inputs[1])-1])
                else if(inputs[1] in subcategories[choosencategory]):
